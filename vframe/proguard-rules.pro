...
-verbose # 混淆时记录日志,包含有类名->混淆后类名的映射关系
-ignorewarning #忽略警告
-printmapping proguardMapping.txt #混淆前后的映射
# 指定混淆时采用的算法后面的参数是一个过滤器，这个过滤器是谷歌推荐的算法，一般不做更改
-optimizations !code/simplification/arithmetic,!field/*,!class/merging/*
-keepattributes *Annotation*,InnerClasses # 保护注解,保留Annotation不混淆
...
# 保持哪些类不被混淆：四大组件，应用类，配置类等等,因为这些子类都有可能被外部调用
...
# 保留在Activity中的方法参数是view的方法,这个主要是在layout中写的onclick方法android:onclick="onClick"，不进行混淆
-keepclassmembers class * extends android.app.Activity {
    public void *(android.view.View);
}

# 保留枚举类enum不被混淆,使用enum类型时需要注意避免以下两个方法混淆，因为enum类的特殊性，以下两个方法会被反射调用
...
#3.webView处理，项目中没有使用到webView忽略即可
-keepclassmembers class fqcn.of.javascript.interface.for.webview {
   public *;
}
...
# 将下面替换成自己的实体类
-keep class frame.vension.kotlin.http.** { *; }
# end
